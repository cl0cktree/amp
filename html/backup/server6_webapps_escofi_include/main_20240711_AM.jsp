...
.TopMenu a {display:inline-block; position:relative; color:white; text-decoration:none; font-size:14px; }
...
/* type_1 ??? ??? ?? */
/*
button.btn.btn-primary{position:absolute; display:block; background:transparent!important; width:28px; height:24px; border:0px solid #5aaf47; left:10px; top:12px;}

button.btn.btn-primary:before{
	content:""; position:absolute; display:block; width:100%; height:7px; background:#5aaf47; border-radius:2px; top:9px; left:0; transform:rotate(45deg); transition:all 0.3s ease-in-out;
}
button.btn.btn-primary:after{
	content:""; position:absolute; display:block; width:100%; height:7px; background:#5aaf47; border-radius:2px; bottom:8px; left:0; transform:rotate(-45deg); transition:all 0.3s ease-in-out;
}

button.btn.btn-primary.collapsed:before{
	content:""; position:absolute; display:block; width:100%; height:7px; background:#5aaf47; border-radius:2px; top:0; left:0; transform:rotate(0deg); transition:all 0.3s ease-in-out;	
}
button.btn.btn-primary.collapsed:after{
	content:""; position:absolute; display:block; width:100%; height:7px; background:#5aaf47; border-radius:2px; bottom:0px; left:0; transform:rotate(0deg); transition:all 0.3s ease-in-out;
}
*/
...
button.btn.btn-primary.collapsed:before{
	content:""; position:absolute; display:block; width:80%; height:7px; background:#5aaf47; border-radius:2px; top:2px; left:0; transform:rotate(45deg); transition:all 0.3s ease-in-out;	
}
button.btn.btn-primary.collapsed:after{
	content:""; position:absolute; display:block; width:80%; height:7px; background:#5aaf47; border-radius:2px; bottom:2px; left:0; transform:rotate(-45deg); transition:all 0.3s ease-in-out;
}
*/

/* type_3 ??? ?? ?? */

button.btn.btn-primary{position:absolute; display:block; background:#5aaf47; width:28px; height:28px; border:2px solid #5aaf47; left:10px; top:12px;}

button.btn.btn-primary:before{
	content:""; position:absolute; display:block; width:100%; height:7px; background:#fff; border-radius:2px; top:9px; left:0; transform:rotate(-45deg); transition:all 0.3s ease-in-out;
}
button.btn.btn-primary:after{
	content:""; position:absolute; display:block; width:100%; height:7px; background:#fff; border-radius:2px; bottom:8px; left:0; transform:rotate(45deg); transition:all 0.3s ease-in-out;
}
button.btn.btn-primary.collapsed:before{
	content:""; position:absolute; display:block; width:85%; height:7px; background:#fff; border-radius:2px; top:4px; left:0; transform:rotate(45deg); transition:all 0.3s ease-in-out;	
}
button.btn.btn-primary.collapsed:after{
	content:""; position:absolute; display:block; width:85%; height:7px; background:#fff; border-radius:2px; bottom:4px; left:0; transform:rotate(-45deg); transition:all 0.3s ease-in-out;
}
...
function init() {
	...
	for (var i=0; i<result.length; i++) {
		...
		if (caption != 'hidden') {
			...
			dotImg.style.marginRight = 20+'px';
...
function checkTrCode() {
	...
	console.log(document.all.topMenuArea.childNodes.length);
	if (trim(tx_code) == "24000" && document.all.topMenuArea.childNodes.length != 5) {
		alert("already login.");
		return false;
	}
	if (tx_code == "") {
...
<table cellspacing=0 cellpadding=0 style="height:100%">
	<tr><td height=10>
		<table cellspacing=0>
			<tr>
				<td id=topMenuArea style="height:50px; color:white;" class=TopMenu>
					<button class="btn btn-primary collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapseContent" aria-expanded="false" aria-controls="collapseContent">
						
					</button>
					<img src="<%= request.getContextPath() %>/images/logo.jpg" style="margin-left:80px;margin-right:100px" border=0>
				</td>
			</tr>
			<tr height=5 bgcolor="#55ad46">
				<td></td>
			</tr>
		</table>
	</td></tr>
	<tr><td>
		<table cellspacing=0 cellpadding=0><tr>
			<td style="display:block; width:100%; max-width:320px; height:100%;">
			<!-- 20240617 style  height: 100% -->		
				<form name=dataForm onsubmit="return checkTrCode()" target=contents method=post style="height: 100%">
				<div class="collapse collapse-horizontal" id="collapseContent">
				<div class="collapse-content p-1" style="background-color: #f8f9fa; border: 1px solid #ddd; border-radius: 5px;">
			<!-- 20240617 style  height: 100% // -->
				<table cellspacing=0>
					...
					<tr>
						<td id=leftScreen height=5 bgcolor=#00aeef style="text-align:center;">
							<B style="color:#000b29;">&nbsp;&nbsp;画面番号</B> :
							<!-- 20240617  style px  -->						
							<input name="trCode" id="screenNumber" tabindex="-1" readonly="true" maxlength="5" type="number" style="width:54px; height:22px; box-sizing:border-box; border: solid 1px black; text-align:center; font-size:13px;" onkeypress="trCode_keypress()">
							<!-- 20240617 style  //-->	
			...
			<td style="width:100%;">
				<table cellspacing=0>
					<tr height=22>
						<td valign=bottom>
							<img src="<%= request.getContextPath() %>/images/icon_toptitle.gif" style="margin-left:10px;">
	...
	<tr><td height=10>
		<table border=1 cellspacing=0 class="table-success table-bordered border-secondary">
