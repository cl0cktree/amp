...
function initFields() {
	...
	for (var i=0; i<xmlParams.length; i++) {
		...
		if (!field) continue;		/* The object which corresponds to a correspondence param Ubs u lyus the Eskip */
		...
		try {
			...
			if (param.getAttribute("require") == "Y") {
				...
				//field.style.borderColor = "#F0F8FF";
			}

			/* The UI the Eonpropertychange addition where when the type is the F3 */
			if (param.getAttribute("UI") == "F3") {
				var func = getFunctionStr(field.onpropertychange);
				field.onpropertychange = new Function(func + ";getDesc();");
				//field.style.backgroundColor = COLOR_MENU_SELECTED_BG;
			}

			/* The input the mask control which when is */
			...
			alert(e.description + "field name duplicate.");
		}
	}
}
...
function makeInMessageBody() {
	...
	try {
		var xmlParams = trInXml.querySelectorAll("param");
		console.log("xmlParams" + xmlParams);
	} catch (e) {
		return false;
	}
	...
	for (var i=0; i<xmlParams.length; i++) {
		...
		var xmlChildParams = param.querySelectorAll("param");	/* The child the param it brings*/
		console.log("xmlChildParams" + xmlChildParams);
		/*The case where the param is having child param*/
		if (xmlChildParams.length > 0) {
...
function w1(name) {
	...
	return parentElement.textContent = message;
 ...
 */
function fKey3(){
	var qtype = event.srcElement.qtype;
	if(!qtype) return;
...
// ↑↑↑ 2016/08/04 EBS 李 預金V3移行試し用修正 END   ↑↑↑
	...
	var slave = document.all[obj.slave];
	var slaveDesc;
	if(slave){
		slaveDesc = document.all[obj.slave+"Lbl"];
		slave.value = "";
		if(!slaveDesc) return;
		slaveDesc.value = "";
	}
	if(obj.qtype == 64 || obj.qtype == 66 || obj.qtype == 67){
		searchF3(obj);
	}else{
...
function searchF3(obj){
	...
	var master = document.all[obj.master];
	var master2 = document.all[obj.master2];
	var master3 = document.all[obj.master3];
	var master4 = document.all[obj.master4];
